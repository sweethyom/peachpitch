...
sudo docker compose -p bluegreen-8083 -f /home/ubuntu/S12P11D201/AITest/docker-compose.bluegreen8083.yml pull
sudo docker compose -p bluegreen-8084 -f /home/ubuntu/S12P11D201/AITest/docker-compose.bluegreen8084.yml pull
...
if [ "$AFTER_S_COLOR" == "8081" ]; then
    ...
    sudo cp ${NGINX_CONF_DIR}/defaultblue.conf $NGINX_CONF_PATH
else
    echo "Green 환경으로 Nginx 설정 파일 교체..."
    sudo cp ${NGINX_CONF_DIR}/defaultgreen.conf $NGINX_CONF_PATH
fi
...
sudo docker compose -p bluegreen-${BEFORE_S_COLOR} -f /home/ubuntu/S12P11D201/BackendTest/docker-compose.bluegreen${BEFORE_S_COLOR}.yml down
sudo docker compose -p bluegreen-${BEFORE_D_COLOR} -f /home/ubuntu/S12P11D201/AITest/docker-compose.bluegreen${BEFORE_D_COLOR}.yml down
